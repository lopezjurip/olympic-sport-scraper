...
if __name__ == '__main__':
    ...
    t1.to_csv('olimpics.csv', sep='\t', encoding='utf-8')

    def process(medal, event, sport, olimpic):
        return {
            'year': olimpic.year,
            'sport': sport.name,
            'event': event.name,
    ...
    full_data = []
    for olimpic in [olimpics[0]]:
        for sport in olimpic.sports:
            for event in sport.events:
                for medal in event.medals:
                    full_data.append(process(medal, event, sport, olimpic))

    t2 = pd.DataFrame(full_data)
    t2.to_csv('full_data.csv', sep='\t', encoding='utf-8')
